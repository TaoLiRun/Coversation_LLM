...
def ques_and_cls_given_items(task, items: list, n, asked_ques: list = None, rest=False):
    response = get_response_method(task.root.model)
    if len(items) <= 1:
        return None
    ...
    print(message)
    rsp = "#" + response(message, model=task.root.model, max_tokens=2000)
    print([rsp])
    ...
    def format_rsp(rsp):
        deepseek_response = get_response_method("deepseek/deepseek-r1")
        ...
        return deepseek_response(message, "deepseek/deepseek-r1/community", max_tokens=500)
...
def cls_given_repo(task, items: list, repo, translate=False, self_repo=True):
    ...
    rsp = response(message, model=task.root.model, max_tokens=len(items)*(task.expected_target_tokens+5))
    print([rsp])
...
def initialize_open_set(task, repo=""):
    ...
    rsp = response(message, model=task.root.model, max_tokens=15*size)
    print([rsp])
    try:
        rsp = set(eval(rsp))
        return list(rsp)
